...
abstract class _AccountManagementStoreBase with Store {
  ...
  Future<void> saveAndContinue() async {
    if (formKey.currentState!.validate()) {
      if (userAction == AccountManagementAction.add ||
        ...
        pendingAccounts.add(item);
      } else {
        if(selectedAccountIndex!=null&&selectedAccount!=null) {
  ...
  @action
  void saveCategories(ObservableMap<int, List<int>?> selections) {
    final List<AddAccountCourseDto> userCategories = [];
    selections.forEach((key, value) {
      userCategories.add(
        AddAccountCourseDto(
          id: key,
          classes: value?.map((e) => AddAccountClassDto(id: e)).toList() ?? [],
        ),
      );
    });
    selectedAccount = selectedAccount?.copyWith(courses: userCategories);
  }
